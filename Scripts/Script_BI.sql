...
IF EXISTS (SELECT *
FROM sys.views
WHERE name = 'v_BI_valor_promedio_mensual_envios')
...
BEGIN
  ...
  BEGIN
    SET @idRango = (
      SELECT rango_horario_id
    FROM QUERY_SQUAD.BI_dim_Rango_Horario
    WHERE @hora >= rango_horario_hora_inicio AND @hora < rango_horario_hora_fin
    );
  ---Separada porque esta comparacion no funciona bien para las 00:00:00
  END
  ELSE
  BEGIN
    SET @idRango = (
      SELECT rango_horario_id
    FROM QUERY_SQUAD.BI_dim_Rango_Horario
    WHERE rango_horario_hora_inicio = '22:00:00.0000000' AND rango_horario_hora_fin = '00:00:00.0000000'
    )
  END
  RETURN @idRango
END;
...
BEGIN
  ...
  GROUP BY QUERY_SQUAD.GetDimTiempoParaFecha(P.pedido_fecha), 
    QUERY_SQUAD.GetDimDiaParaFecha(P.pedido_fecha), 
    L.local_id,
    QUERY_SQUAD.GetRangoHorario(CONVERT(TIME,P.pedido_fecha)), 
    QUERY_SQUAD.GetRangoEtario(U.usuario_fecha_nac,p.pedido_fecha),
    QUERY_SQUAD.GetRangoEtario(R.repartidor_fecha_nac,pedido_fecha),
    Lo.localidad_id,
    C.categoria_local_id,
    R.repartidor_tipo_movilidad_id,
    M.medio_de_pago_tipo_id,
...
BEGIN
  ...
  GROUP BY QUERY_SQUAD.GetDimTiempoParaFecha(E.envio_mensajeria_fecha),
    QUERY_SQUAD.GetDimDiaParaFecha(E.envio_mensajeria_fecha),
    QUERY_SQUAD.GetRangoHorario(CONVERT(TIME,E.envio_mensajeria_fecha)),
    E.envio_mensajeria_localidad_id,
    R.repartidor_tipo_movilidad_id,
...
BEGIN
  ...
  GROUP BY QUERY_SQUAD.GetDimTiempoParaFecha(R.reclamo_fecha),
    P.pedido_local_id, 
    QUERY_SQUAD.GetDimDiaParaFecha(R.reclamo_fecha),
    QUERY_SQUAD.GetRangoHorario(CONVERT(TIME,R.reclamo_fecha)), 
    QUERY_SQUAD.GetRangoEtario(O.operador_reclamo_fecha_nac, R.reclamo_fecha),
...
AS
  ...
  WHERE tiempo_anio = c.tiempo_anio
    AND tiempo_mes = c.tiempo_mes
    AND localidad_nombre = c.localidad_nombre
...
AS
  ...
  GROUP BY TM.tipo_movilidad_tipo, 
    D.dia_nombre, 
    RH.rango_horario_hora_inicio, 
    RH.rango_horario_hora_fin
GO
...
AS
  SELECT L.localidad_nombre AS Localidad ,T.tiempo_anio AS Anio ,T.tiempo_mes AS Mes
    ,SUM(H_Pedidos.hechos_pedidos_valor_promedio_envio * H_Pedidos.hechos_pedidos_cantidad_pedidos)/SUM(H_Pedidos.hechos_pedidos_cantidad_pedidos) AS ValorPromedioEnvio
  --Por como se agurapan los datos hay que hacer asi el calulo de promedio
  FROM QUERY_SQUAD.BI_Hechos_Pedidos AS H_Pedidos
...
AS
  ...
  SELECT
    ...
    ,SUM(cantidad_entregas) * 100/CAST(
			(SELECT SUM(hechos_pedidos_cantidad_pedidos)
    FROM QUERY_SQUAD.BI_Hechos_Pedidos
    WHERE hechos_pedidos_tiempo_id = t.tiempo_id AND hechos_pedidos_estado_pedido_id = 1) +  
			(SELECT SUM(hechos_mensajeria_cantidad_envios)
    FROM QUERY_SQUAD.BI_Hechos_Mensajeria
    WHERE hechos_mensajeria_tiempo_id  = t.tiempo_id AND hechos_mensajeria_estado_envio_id = 1)
		 AS FLOAT) AS porcentajeDelTotalMensual
  ---Esto es bastante feo pero si lo hago en una funcion tarda anios despues la vista 
  FROM
    (
      SELECT
  ...
  GROUP BY t.tiempo_anio,
    t.tiempo_mes,
    R.rango_etario_minimo ,
    R.rango_etario_maximo,
    L.localidad_nombre,
    t.tiempo_id
GO
